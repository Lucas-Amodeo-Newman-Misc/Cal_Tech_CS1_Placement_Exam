...
class Hanoi():
    ...
    def move(self, start_peg, target_peg):
        ...
        if self._state[target_peg]: # if the target peg has contents already, checks the intended move for legality
        ...
        self.display() # displays state - a convenient time to do this due to the recursive nature of the program

    # Solves the puzzle by transfering the entire stack on peg A to peg B
    def solve(self):
        self._transfer_stack("A", "B", len(self._state["A"]))

    # Recursive algorithm for transfering a stack of arbitrary size from one peg to another.
